...
### Format Data
# Read in data
df = pd.read_csv('ValidationTestDatasetBigger_x16.csv')
# df = pd.read_csv('ValidationTestDatasetBigger.csv')

# labels = np.array(df.pop('label'))

# Reorganize data
cols_to_move = ['Category','LandType']
target_names = np.array(list(df['LandType'].values))
...
df = df.drop(columns=['ImageX','ImageY'])
# df = df.drop(['ImageX','ImageY'], axis=1)
